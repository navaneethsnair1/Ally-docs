...
const IGuideline_1 = require("../engine/IGuideline");
const IRule_1 = require("../engine/IRule");
const ReporterManager_1 = require("./ReporterManager");
...
class ACReporterXLSX {
    ...
    generateSummary(config, rulesets, endReport, summaryData) {
        return __awaiter(this, void 0, void 0, function* () {
            let storedReport = ReporterManager_1.ReporterManager.uncompressReport(summaryData[0]);
    ...
    static createOverviewSheet(config, compressedScans, workbook) {
        ...
        for (const compressedScan of compressedScans) {
            let storedScan = ReporterManager_1.ReporterManager.uncompressReport(compressedScan);
            if (startScan === 0)
    ...
    static createIssueSummarySheet(config, policyInfo, compressedScans, workbook) {
        ...
        for (let i = 0; i < compressedScans.length; i++) {
            let storedScan = ReporterManager_1.ReporterManager.uncompressReport(compressedScans[i]);
            let counts = storedScan.engineReport.summary.counts;
            violations += counts.violation;
            needsReviews += counts.potentialviolation + counts.manual;
            recommendations += counts.recommendation + counts.potentialrecommendation;
        ...
        let level4A = [];
        for (const compressedScan of compressedScans) {
            let scan = ReporterManager_1.ReporterManager.uncompressReport(compressedScan);
            for (const issue of scan.engineReport.results) {
                ...
                else if (issuePolicyInfo.tkLevels.includes(IGuideline_1.eToolkitLevel.LEVEL_TWO)) {
                ...
                else if (issuePolicyInfo.tkLevels.includes(IGuideline_1.eToolkitLevel.LEVEL_THREE)) {
                ...
                else if (issuePolicyInfo.tkLevels.includes(IGuideline_1.eToolkitLevel.LEVEL_FOUR)) {
                ...
                if (issue.value[1] !== IRule_1.eRuleConfidence.PASS) {
                    ++levelCounts[0];
                }
                if (issue.ignored) {
                    ++levelCounts[4];
